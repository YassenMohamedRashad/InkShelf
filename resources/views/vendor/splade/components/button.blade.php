...
<SpladeButton {{ $attributes->only(['v-bind:spinner', ':spinner']) }}>
    <template #default="button">
        <{{ $type === 'link' ? 'Link' : 'button' }} :disabled="button.spinner" {{ $attributes->class([
            ...
            'bg-orange-color hover:bg-dark-orange text-white border-transparent focus:border-orange-color focus:ring-orange-color' => !$customStyling && $primary,
            'bg-red-500 hover:bg-red-700 text-white border-transparent focus:border-red-700 focus:ring-red-200' => !$customStyling && $danger,
            'bg-white hover:bg-gray-100 text-gray-700 border-gray-300 focus:border-indigo-300 focus:ring-orange-color' => !$customStyling && $secondary,
        ])
